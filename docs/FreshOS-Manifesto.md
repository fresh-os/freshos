...
GPU composition is the goal. On the Pi 4, software compositing with damage tracking comes first, and a GPU driver follows only if measurement shows software is too slow (see the v1 scope's compositing gate). The compositor owns the display pipeline and provides deterministic frame scheduling. Applications submit scene graphs or buffers; the compositor handles presentation. Spatial workspace transitions and physics are compositor-level concerns.
...
### Boot Target: UEFI on the Raspberry Pi 4

One cheap, fixed, widely owned board: the Raspberry Pi 4 (aarch64), booted through community UEFI firmware, which provides a framebuffer and basic services. QEMU `virt` on Apple Silicon is the development loop. The Pi 5 comes next. x86_64 was dropped (decision 0003).
...
**Tier 3: Virtualised ARM Linux.** Hardware-assisted virtualisation (Arm EL2) for near-native performance. Runs Linux applications or other OS instances built for ARM. Less observable than Tiers 1 and 2 — the VM is internally opaque — but fully capability-controlled at the boundary.
...
**SIMD as a system concern.** NEON on the Pi's Cortex-A72, and equivalent vector units elsewhere, are available for system services — the audio mixer, the compositor's scene composition, image format conversion. These aren't application optimisations; they're system-level performance multipliers that benefit everything.
...
The staged plan lives in [`FreshOS-v1-Scope.md`](FreshOS-v1-Scope.md): a ladder from an honest kernel on the Pi 4, through storage, a usable desktop, games and networking, to the web. Each rung ends with a demo on real hardware.

Everything beyond v1, including the server, embedded and automotive editions and the physical-world vision, is in [`FreshOS-Horizon.md`](FreshOS-Horizon.md).
...
*FreshOS Manifesto v4 — September 2026. Retargeted to aarch64 and the Raspberry Pi 4; editions and the physical-world vision moved to `FreshOS-Horizon.md`.*
*Steve*
