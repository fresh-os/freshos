...
- **`init`'s table** (`userbins/init/src/main.rs`) lists each service: name, binary, grants, argument, restart delay, and whether it is optional. An optional service whose binary is absent is skipped silently; any other refused spawn is logged with the service and its binary, as `[init] cannot start pong (PONG.ELF): NotFound`, and `init` carries on.
