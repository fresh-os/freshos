/// 8×8 bitmap font — full printable ASCII (32–126).
///
/// Each glyph is 8 bytes, one byte per row, MSB = leftmost pixel.
/// Based on the classic IBM PC / CGA 8×8 character set (public domain).

pub const CHAR_WIDTH: usize = 8;
pub const CHAR_HEIGHT: usize = 8;

/// Return the 8-byte glyph bitmap for `c`. Unknown chars map to a filled box.
pub fn glyph(c: char) -> &'static [u8; CHAR_HEIGHT] {
    let i = c as usize;
    if i >= 32 && i < 127 {
        &FONT[i - 32]
    } else {
...
const UNKNOWN: [u8; 8] = [0xFF, 0x81, 0x81, 0x81, 0x81, 0x81, 0x81, 0xFF];
